...
| `favorites` | Одну сохранённую пользователем ссылку на объявление | `user_id`, `listing_id`, `created_at` |
| `listing_views` | Учтённые открытия одного объявления одним пользователем | `user_id`, `listing_id`, `last_viewed_at`, `view_count` |
...
3. Выберите `data/apartments.sqlite3` внутри запущенной копии проекта (абсолютный путь можно получить командой из README).
4. В **Database Structure / Структура БД** видны семь таблиц и индексы. Раскройте таблицу, чтобы увидеть её столбцы.
5. В **Browse Data / Просмотр данных** выберите нужную таблицу из списка. Пользователи, бронирования, избранное и просмотры появляются после соответствующих действий через сайт.
...
Сведения о сессиях, CSRF, сроках, HTTPS и ограничении входа находятся в [README](../README.md). Освобождение/перепродажа и персонализация разобраны ниже. ML не реализован.
...
## Пятый этап: favorites и listing_views

Миграция 5 добавляет **две** таблицы; существующие пять таблиц не меняются. При старте C++ видит `PRAGMA user_version = 4`, выполняет только `005_personalization.sql` внутри транзакции и получает версию 5. Уже применённые файлы 1–4 остаются прежними. Генератор недвижимости новые личные таблицы не заполняет, не удаляет и не очищает.

`favorites` хранит факт «пользователь сохранил именно это объявление»:

| Поле | Смысл |
| --- | --- |
| `user_id INTEGER NOT NULL` | Внешний ключ на users.id, берётся из проверенной сессии |
| `listing_id INTEGER NOT NULL` | Внешний ключ на listings.id, номер сохранённого предложения |
| `created_at TEXT NOT NULL` | Дата первого добавления в UTC, автоматически задаётся SQLite |

Отдельного поля id нет: **PRIMARY KEY(user_id, listing_id)** делает пару уникальной. Пользователи A и B могут сохранить одно объявление, но у A не может быть двух одинаковых сохранений. `NOT NULL` не позволяет обойти уникальность отсутствующим номером. Номер объекта здесь не используется вместо объявления: если предложение закрыто, избранное по-прежнему ведёт в его историю, а не на новую перепродажу.

`listing_views` хранит одну сводную строку на каждую такую же пару:

| Поле | Смысл |
| --- | --- |
| `user_id`, `listing_id` | Внешние ключи; вместе составляют первичный ключ |
| `last_viewed_at INTEGER NOT NULL` | Время последнего **учтённого** открытия, целые секунды с 1 января 1970 UTC |
| `view_count INTEGER NOT NULL` | Положительный целый счётчик учтённых открытий, первоначально 1 |

Вместо бесконечной таблицы событий здесь один счётчик и одно время для пары. Даты каждого прежнего открытия отдельно не хранятся. IP, устройство, cookies и отпечаток браузера в этих таблицах отсутствуют. В UI секунды умножаются на 1000 для JavaScript Date, затем показывается привычная местная дата. История доступна только владельцу и не имеет API редактирования времени или счётчика.

Все внешние ключи используют `ON DELETE RESTRICT`, чтобы ручное удаление пользователя/объявления не разрушало личные ссылки. Как прежде, C++ включает `PRAGMA foreign_keys = ON` **на каждом соединении** до транзакции. Новые таблицы не дают дополнительных прав на бронирование, освобождение или перепродажу.

## Идемпотентное избранное и защищённая запись просмотра

Идемпотентность означает: один и несколько одинаковых запросов приводят к одному состоянию. Сервер получает **явное намерение**, а не команду «поменяй на противоположное»:

- `PUT /api/favorites/7` с `{}` — объявление должно быть сохранено;
- `DELETE /api/favorites/7` с `{}` — объявление должно отсутствовать в избранном текущего пользователя.

`auth.cpp` проверяет сессию, CSRF и форму запроса. `setFavorite` в `personalization.cpp` проверяет наличие объявления и выполняет одну из команд ниже. Это **фрагменты серверной записи с параметрами, а не команды для ручной проверки рабочей базы**:
...
Первое `?` — пользователь из сессии; второе — проверенный положительный номер объявления из URL. При повторном INSERT конфликт пары явно означает «ничего не менять», включая дату. DELETE может удалить ноль строк — это нормальный повтор, ответ всё равно 200. Если A и B отправят DELETE одного id, каждый действует только на своей паре. Подставить чужой user_id через JSON/query нельзя. Установка и удаление одновременно дают итог последней зафиксированной операции; две установки не создают дубль. Гарантии базы работают для независимых соединений.

Просмотр записывается только отдельным `POST /api/listings/7/view` с `{}` и CSRF. `listing.js` отправляет его после успешной загрузки подробной страницы вошедшим пользователем, один раз за открытие страницы. Каталог и GET `/api/listings/7` не вызывают запись. После неудачной записи показывается сообщение, POST автоматически не повторяется.

Внутри `recordView` используется атомарный UPSERT:
...
`UPSERT` означает «вставь, а если пара существует — обнови при условии». `excluded.last_viewed_at` — новое время, которое SQLite собиралась вставить. `strftime('%s','now')` берёт часы сервера; `CAST ... AS INTEGER` превращает строковое представление времени в число. Клиент не может прислать время раньше или позже.

Если с прошлого учтённого открытия прошло **не меньше 1800 секунд**, меняются время и счётчик. Внутри окна не меняется ничего: частое обновление страницы не сдвигает окно бесконечно. `sqlite3_changes` возвращает 1 для вставки/учтённого обновления и 0 для повтора. API передаёт это как `counted: true/false`. При одновременном первом открытии одной пары ровно одна запись увеличивает счётчик, что проверяется HTTP-тестом.

Обе операции используют прежний `writeTransaction`: BEGIN IMMEDIATE, проверка существования объявления, SQL, COMMIT. При ошибке — полный ROLLBACK; максимум четыре попытки всей транзакции, до 250 мс ожидания SQLite за попытку, паузы 25/50/75 мс. Длительная блокировка даёт 503, а не ложный успех. Так же продолжают работать три операции бронирований.

## Чтение личных списков и индексы

`personalPage` получает user_id из сессии. Для избранного читает:
...
Для истории — аналогично:

```sql
SELECT COUNT(*) FROM listing_views WHERE user_id = ?;

SELECT listing_id, last_viewed_at, view_count
FROM listing_views WHERE user_id = ?
ORDER BY last_viewed_at DESC, listing_id DESC
LIMIT ? OFFSET ?;
```

Сначала отбираются номера одной страницы (24 по умолчанию, максимум 100), **затем одним запросом** `listingsByIds` получает объявления и фото для всего набора через `IN (?, ?, ...)` и `LEFT JOIN property_photos`. Фиксированное количество знаков `?` строится из числа id, сами номера всегда связываются через bind. Дополнительный пакетный запрос отмечает избранные в истории. COUNT и чтения выполняются в одной транзакции чтения, поэтому относятся к одному снимку данных. Несколько фото не увеличивают количество карточек на странице.

Порядок «дата по убыванию, затем id объявления по убыванию» стабилен при равных датах. OFFSET = `(page - 1) * page_size`. Изменение списка между отдельными HTTP-запросами может сдвинуть страницы — это обычная пагинация по текущему состоянию, а не сохранённый навсегда снимок. После удаления последней карточки на странице интерфейс переходит к последней существующей странице.

| Индекс | Назначение |
| --- | --- |
| PRIMARY KEY `(user_id, listing_id)` в обеих таблицах | Уникальность пары, точный поиск/удаление, проверка наличия в избранном |
| `favorites_user_recent(user_id, created_at DESC, listing_id DESC)` | Страница и последние 100 сохранённых объявлений в нужном порядке |
| `views_user_recent(user_id, last_viewed_at DESC, listing_id DESC)` | Страница и последние 100 просмотров |
| `favorites_listing(listing_id)`, `views_listing(listing_id)` | Проверка ссылок при попытке удалить объявление |

Индексы соответствуют нашим запросам: сначала равенство пользователя, затем порядок внутри его списка. Это не универсальная гарантия ускорения любых запросов; индексы занимают место и обновляются при записи.

## Рекомендации без ML: правила и пример пользователя

Алгоритм находится в **Database::recommendations** в `src/personalization.cpp`. Он не обучается и не предсказывает цены. C++ начисляет баллы по заранее выбранным правилам:

1. Берёт до **100 последних избранных** с весом **3** и до **100 последних просмотренных** с весом **1**. Сохранённое объявление не учитывается ещё раз как просмотренное, даже если оно вне первых 100 избранных. Счётчик повторных просмотров не увеличивает вес. Это сдерживает влияние многократных обновлений одной страницы.
2. Район, текущая цена и тип сделки берутся из связанных объявлений/объектов. Сохранённое закрытое объявление тоже может описывать интерес пользователя. После ручного изменения цены в БД предпочтение будет рассчитано по новой цене: отдельного снимка цены просмотра нет.
3. Кандидаты — только available, вне **всего** избранного пользователя и с продавцом, отличным от пользователя (либо без продавца у демоданных). Просмотренные объявления не исключаются: знакомый вариант может встретиться снова.
4. Для кандидата сравниваются **только сигналы того же типа сделки**. Относительное расстояние цены `d = abs(цена кандидата − цена сигнала) / цена сигнала`. Это сравнение с ценой конкретного интересующего объявления, а не общая средняя аренды и продажи.

| Признак | Баллы одного сигнала до умножения на вес |
| --- | --- |
| Тот же район | 4, иначе 0 |
| Разница цены до 10% включительно | 4 |
| Разница больше 10%, до 25% включительно | 2 |
| Разница больше 25%, до 50% включительно | 1 |
| Разница больше 50% | 0 |

Баллы района и цены складываются, затем умножаются на вес сигнала. **Итог = целая часть от `100 × сумма взвешенных баллов / сумма весов сигналов данного типа сделки`**. При отсутствии сигналов этого типа — 0. Диапазон 0–800; это оценка совпадения правил, не вероятность. Нормализация отдельно для rent/sale не позволяет большому числу просмотров продаж менять оценки аренды. Очень слабый вклад может округлиться вниз до 0.

**Конкретный учебный пример из небольшого тестового набора.** Алиса просмотрела аренду в Кентроне за 100 000 драмов (вес 1) и сохранила аренду в Арабкире за 100 000 (вес 3). Это специально заданные цены теста, не утверждение о стоимости реальных квартир:

- Кандидат A: Арабкир, 100 000. От просмотра Кентрона — 4 за цену; от избранного — `(4 район + 4 цена) × 3 = 24`. Итог: `100 × (4 + 24) / 4 = 700`.
- Кандидат B: Кентрон, 110 000. От просмотра — 8; от избранного — `4 за близкую цену × 3 = 12`. Итог: `100 × (8 + 12) / 4 = 500`.
- Кандидат C: Кентрон, 400 000. Только 4 за район просмотра. Итог — 100. Он ниже B, потому что цена значительно отличается.
- Продажа в Кентроне даже за те же численные 100 000 получает 0: у Алисы пока нет сигналов **продажи**. Если появится просмотр продажи в Арабкире за 50 млн, он начнёт влиять только на продажи.

Таким образом, избранный Арабкир перевешивает единичный просмотр Кентрона, а близость цены различает объекты одного района. Эти отношения проверяются тестом по известным примерам, без копирования формулы в проверяющий код.

Кандидаты сортируются по оценке убывающе, при равенстве по id возрастающе. Поддерживается не больше **limit** лучших записей в памяти: по умолчанию 6, максимум 24. Если положительных совпадений мало, оставшиеся места занимают варианты с нулевой оценкой в порядке id. Дублей нет: запрос выбирает каждое объявление один раз. Если доступных вариантов меньше limit, ответ короче.

Пояснения выводятся только при подтверждающем совпадении:

- «Похожая цена в интересующем вас районе» — есть хотя бы один сигнал того же типа, того же района и разницей цены до 25%;
- «В интересующем вас районе» — район совпал, предыдущего сочетания нет;
- «Близкий ценовой диапазон» — положительную оценку дала близость цены без совпадения района;
- «Доступное жильё из каталога» — нулевая оценка, обычное дополнение.

У гостя и нового пользователя все оценки нулевые, `personalized = false`. Этот флаг также false, если в возвращённой подборке не нашлось положительного совпадения. UI прямо пишет «Обычная подборка доступного жилья из каталога», даже если над блоком общий заголовок «Вам может понравиться».

Обращений к базе мало: один запрос сигналов (для вошедшего), один проход по минимальным полям доступных кандидатов, один пакетный запрос деталей/фото выбранных id. Запроса на каждую карточку нет. Всё читается одним снимком SQLite. Объём сигналов ограничен 200, память лучших кандидатов — 24. Сам проход по доступным объявлениям **не ограничен 1000** и работает с будущими перепродажами; время растёт примерно как число доступных объявлений × число сигналов. Для большого каталога понадобятся измерения и предварительный отбор. Сейчас это понятный алгоритм для учебного набора, не обещание производительности при миллионах строк.

## Новые запросы для DB Browser — только чтение

Полный файл: [personalization-queries.sql](personalization-queries.sql). В примерах `user_id = 1`; замените 1 своим номером из users. Запросы ниже не меняют данные и не показывают хеши или токены.
...
`datetime(..., 'unixepoch')` делает из числа секунд читаемую дату UTC. `UNION ALL` просто объединяет две строки результата. Пустой список после миграции нормален: генератор не выдумывает действия пользователя.

## Ремонт и безопасное обновление демонстрационных данных (схема 6)

Миграция `006_demo_renovation.sql` добавляет в `properties` `renovation` и `demo_revision`, не пересоздавая таблицы. Первый столбец — устойчивое значение состояния ремонта, второй — отметка, что отдельная команда уже обновила демообъект. Новые значения передаются через SQL-параметры; CHECK не допускает произвольные состояния.

Ремонт относится к недвижимости, поэтому при перепродаже остаётся тем же: новое объявление ссылается на прежний property_id. `price_at_booking` хранит прежнюю цену сделки и не зависит от последующего генератора. Обычный запуск не меняет существующие объекты; для обновления требуется отдельная команда и точное совпадение со старым эталоном.

Полные коэффициенты, обе команды, резервное копирование SQLite, транзакция проверки/обновления, фактическая статистика и запросы COUNT(DISTINCT ...) приведены в [demo-generator.md](demo-generator.md).
